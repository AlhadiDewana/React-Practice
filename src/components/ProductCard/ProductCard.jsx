...
export default function ProductCard({product}) {
  ...
  return (
    <Link to={`/products/${product.slug}` ?? ''} className="flex flex-col max-w-[370px] flex-wrap p-[16px] bg-[#D8DBBD] hover:ring-opacity-40 active:ring-5 active:ring-[#6247eb] hover:ring-4 active:ring-2 active:ring-opacity-90" >
         <img src={product?.imageUrl ?? ''} alt={product?.name ?? 'No Title'} className="block max-h-[300px] mb-4 object-cover" />
          <div className="flex flex-col gap-2">
              ...
              <span className="block font-medium text-[14px] text-gray-500">{product?.category ?? "Uncategorized"}</span>
